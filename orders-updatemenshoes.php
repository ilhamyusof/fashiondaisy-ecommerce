...
if(isset($_SESSION['cart'])) {
  ...
  foreach($_SESSION['cart'] as $product_id => $quantity) {
    ...
    $result = $mysqli->query("SELECT * FROM menshoes WHERE id = ".$product_id);
    ...
    if($result){
      ...
      if($obj = $result->fetch_object()) {
        ...
        if($query){
          ...
          if($mysqli->query("UPDATE menshoes SET qty = ".$newqty." WHERE id = ".$product_id)){
...
unset($_SESSION['cart']);
header("location:checkoutmenshoes.php");
